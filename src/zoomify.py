...
input_files = glob.glob(os.path.join(directories[0],'*.*'))
if len(input_files) > 0:
    for file in input_files:
        file_prefix = str(os.path.split(file)[1]).split('.')
        file_dir = os.path.join(directories[1], file_prefix[0])
        os.makedirs(file_dir)
        # Open image and determine size, tiles and zoom levels required
        input_path = os.path.join(directories[0],file)
        ...
        for loop_zoom in xrange(0, int(max_zoom_level)):
            ...
            for loop_row in xrange(0, int(rows_required)):
                ...
                for loop_col in xrange(0, int(cols_required)):
                    ...
                    tile_subdir = os.path.join(file_dir, TILE_SUBDIR_PREFIX+tile_subdir_suffix)
        ...
        with open(os.path.join(file_dir, 'ImageProperties.xml'), 'w') as f:
